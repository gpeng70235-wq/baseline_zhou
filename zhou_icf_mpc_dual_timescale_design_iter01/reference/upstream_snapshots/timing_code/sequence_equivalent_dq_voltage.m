...
if mode=="legacy_selection_angle"
    ...
    u_dq=zhou_iter11_ref.math.park(command.reference_ab_V,theta_sample);return
end
...
u_dq=[0;0];for j=1:numel(d),u_dq=u_dq+d(j)/Ts*zhou_iter11_ref.math.park(vectors.ab_V(ids(j)+1,:),angles(j));end
end
