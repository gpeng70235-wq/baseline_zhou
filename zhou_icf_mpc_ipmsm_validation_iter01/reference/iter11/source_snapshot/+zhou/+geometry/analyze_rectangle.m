...
classification = zhou.geometry.classify_case(rect, tolerance);
selection = struct();
midpoint = struct();

switch classification.case_id
    case 1
        command = zhou.modulation.case1_command(rect.Ts, vectors);
    case 2
        selection = zhou.geometry.select_savv( ...
            classification.line_hits, rect.center_ab, vectors, sector_tolerance);
        midpoint = zhou.geometry.case2_midpoint(rect, ...
        ...
        command = zhou.modulation.case2_command(midpoint.midpoint_ab, ...
            selection.vector_id, rect.Ts, vectors, duty_tolerance);
    case 3
        command = zhou.modulation.case3_command( ...
            rect.center_ab(:).', rect.Ts, vectors, sector_tolerance, duty_tolerance);
end

reference_dq = zhou.math.park(command.reference_ab_V, rect.theta);
...
[reference_inside, reference_residual] = ...
    zhou.geometry.point_in_rectangle(command.reference_ab_V, rect, tolerance);
