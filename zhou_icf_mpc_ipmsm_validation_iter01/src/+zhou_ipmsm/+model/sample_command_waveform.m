...
assert(abs(sample_count*sample_period-Ts) <= max(tolerance,100*eps(Ts)), ...
    'ZhouIPMSM:WaveformGridMismatch', ...
...
assert(numel(durations)==numel(ids) && all(isfinite(durations)), ...
    'ZhouIPMSM:InvalidSequence','Invalid switching sequence for waveform replay.');
assert(all(durations >= -tolerance), 'ZhouIPMSM:NegativeDwell', ...
...
assert(abs(sum(durations)-Ts) <= max(tolerance,100*eps(Ts)), ...
    'ZhouIPMSM:DwellSumFailure','Waveform replay dwell times do not sum to Ts.');
...
k1 = zhou_ipmsm.model.pmsm_derivative(state,voltage_ab,omega_e,motor,delta_voltage_ab);
k2 = zhou_ipmsm.model.pmsm_derivative(state+dt*k1/2,voltage_ab,omega_e,motor,delta_voltage_ab);
k3 = zhou_ipmsm.model.pmsm_derivative(state+dt*k2/2,voltage_ab,omega_e,motor,delta_voltage_ab);
k4 = zhou_ipmsm.model.pmsm_derivative(state+dt*k3,voltage_ab,omega_e,motor,delta_voltage_ab);
