function [next_state, applied_average_dq, integration_audit] = ...
        integrate_command(state, command, vectors, omega_e, motor, Ts, tolerance, delta_voltage_ab, max_step_s)
...
for segment = 1:numel(ids)
    ...
    applied_average_dq = applied_average_dq + ...
        dt/Ts * zhou_ipmsm.math.park(voltage_ab, theta_mid);

    count = max(1,ceil(dt/max_step_s));
    h = dt/count;
    for substep = 1:count
        k1 = zhou_ipmsm.model.pmsm_derivative(next_state,voltage_ab,omega_e,motor,delta_voltage_ab);
        k2 = zhou_ipmsm.model.pmsm_derivative(next_state+h*k1/2,voltage_ab,omega_e,motor,delta_voltage_ab);
        k3 = zhou_ipmsm.model.pmsm_derivative(next_state+h*k2/2,voltage_ab,omega_e,motor,delta_voltage_ab);
        k4 = zhou_ipmsm.model.pmsm_derivative(next_state+h*k3,voltage_ab,omega_e,motor,delta_voltage_ab);
        next_state = next_state + h*(k1 + 2*k2 + 2*k3 + k4)/6;
    end
    rk4_substeps = rk4_substeps+count;
    elapsed = elapsed + dt;
end
...
integration_audit.rk4_substeps = rk4_substeps;
end
