...
if isfield(input,'Fhat_override_dq')
    Fhat=input.Fhat_override_dq(:);
elseif cfg.assumptions.estimator=="fliess_join_2013_algebraic_integral"
    Fhat=zhou_ipmsm.controller.estimate_F_algebraic(input.current_history_dq, ...
        input.applied_history_dq,alpha,cfg.paper.Ts_s);
else
    Fhat = zhou_ipmsm.controller.estimate_F(input.current_dq, ...
...
if cfg.assumptions.candidate_voltage_frame_mode=="execution_segment_midpoint"
    pending_voltage=zhou_ipmsm.controller.sequence_equivalent_dq_voltage(input.pending_command, ...
...
predicted_k1 = zhou_ipmsm.controller.predict_k1(input.current_dq, Fhat, ...
    pending_voltage, alpha, cfg.paper.Ts_s);
V = zhou_ipmsm.controller.compute_V_terms(input.reference_dq, ...
...
rect = zhou_ipmsm.geometry.build_rectangle(V(1), V(2), cfg.Jd_limit, ...
...
command.reference_dq_execution = zhou_ipmsm.controller.sequence_equivalent_dq_voltage(command, ...
    cfg.vectors,input.theta_e,input.omega_e,1,cfg.paper.Ts_s,cfg.assumptions.candidate_voltage_frame_mode);

predicted_k2 = zhou_ipmsm.controller.predict_k2(predicted_k1, Fhat, ...
    command.reference_dq_execution, alpha, cfg.paper.Ts_s);
[Jd_eq5, Jq_eq5] = zhou_ipmsm.controller.independent_costs( ...
    input.reference_dq, predicted_k2);
[Jd_eq7, Jq_eq7] = zhou_ipmsm.controller.independent_costs(input.reference_dq,predicted_k2);

crosscheck_error = max(abs([Jd_eq5-Jd_eq7, Jq_eq5-Jq_eq7]));
assert(crosscheck_error <= 1e-9, 'ZhouIPMSM:CostCrosscheckFailure', ...
