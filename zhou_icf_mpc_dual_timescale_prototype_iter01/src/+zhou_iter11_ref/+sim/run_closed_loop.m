...
vectors = zhou_iter11_ref.inverter.voltage_vectors(a.dc_bus_V, a.voltage_vector_scale);
controller_paper=p;
...
assert(abs(waveform_samples_per_cycle*a.waveform_sample_period_s-Ts) <= ...
    a.time_tolerance_s,'ZhouIter11Ref:WaveformGridMismatch', ...
...
if a.preinitialize_fixed_speed_controller
    ...
    equilibrium_ab=zhou_iter11_ref.math.inv_park(equilibrium_dq,state(3)).';
    if norm(equilibrium_ab)<=a.voltage_tolerance_V
        pending=zhou_iter11_ref.modulation.case1_command(Ts,vectors);
    else
        pending=zhou_iter11_ref.modulation.case3_command(equilibrium_ab,Ts,vectors, ...
...
for k = 1:steps
    ...
    timer = tic;
    if method == "ICF_MPC"
        control = zhou_iter11_ref.controller.icf_mpc_step(input, cfg);
    else
        control = zhou_iter11_ref.baseline.fcs_mpcc_step(input, cfg);
    end
    ...
    abc = zhou_iter11_ref.model.phase_currents(current_dq, state(3));
    ia_A(k) = abc(1); ib_A(k) = abc(2); ic_A(k) = abc(3);
    torque_Nm(k) = zhou_iter11_ref.model.electromagnetic_torque(current_dq(2), p);
    ...
    if ~pending.legal
        error('ZhouIter11Ref:AppliedIllegalCommand', ...
    ...
    if a.inverter_disturbance_model=="paper_2us_average_deadtime_per_commutated_leg"
        [deadtime_delta_ab,deadtime_audit]=zhou_iter11_ref.inverter.deadtime_voltage_error( ...
    ...
    deadtime_delta_dq=zhou_iter11_ref.math.park(deadtime_delta_ab,state(3));
    ...
    [waveform_states, waveform_audit] = zhou_iter11_ref.model.sample_command_waveform( ...
    ...
    [next_state, applied_average_dq, integration_audit] = ...
        zhou_iter11_ref.model.integrate_command(state, pending, vectors, omega_e, p, ...
...
waveform_torque_Nm = zhou_iter11_ref.model.electromagnetic_torque(waveform_iq_A,p);
