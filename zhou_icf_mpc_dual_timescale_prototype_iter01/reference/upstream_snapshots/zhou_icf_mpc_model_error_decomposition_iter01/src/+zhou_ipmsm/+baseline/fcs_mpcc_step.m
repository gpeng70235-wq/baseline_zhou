...
for vector_id = 0:6
    voltage_dq = zhou_ipmsm.math.park(cfg.vectors.ab_V(vector_id + 1, :), input.theta_e);
    ...
    [candidate_Jd(vector_id + 1), candidate_Jq(vector_id + 1)] = ...
        zhou_ipmsm.controller.independent_costs(input.reference_dq, predicted_k2);
...
command.reference_dq_at_selection = zhou_ipmsm.math.park(selected_ab, input.theta_e);
...
output.V_dq = zhou_ipmsm.controller.compute_V_terms(input.reference_dq, ...
