...
if cfg.assumptions.estimator=="fliess_join_2013_algebraic_integral"
    Fhat=zhou_iter11_ref.controller.estimate_F_algebraic(input.current_history_dq, ...
        input.applied_history_dq,alpha,cfg.paper.Ts_s);
else
    Fhat = zhou_iter11_ref.controller.estimate_F(input.current_dq, ...
...
if cfg.assumptions.candidate_voltage_frame_mode=="execution_segment_midpoint"
    pending_voltage=zhou_iter11_ref.controller.sequence_equivalent_dq_voltage(input.pending_command, ...
...
predicted_k1 = zhou_iter11_ref.controller.predict_k1(input.current_dq, Fhat, ...
    pending_voltage, alpha, cfg.paper.Ts_s);
V = zhou_iter11_ref.controller.compute_V_terms(input.reference_dq, ...
...
rect = zhou_iter11_ref.geometry.build_rectangle(V(1), V(2), cfg.Jd_limit, ...
    cfg.Jq_limit, alpha(1), alpha(2), cfg.paper.Ts_s, geometry_theta, ...
    cfg.assumptions.geometry_tolerance);
geometry = zhou_iter11_ref.geometry.analyze_rectangle(rect, cfg.vectors, ...
...
command.reference_dq_at_selection = zhou_iter11_ref.math.park(command.reference_ab_V,input.theta_e);
command.theta_at_selection = input.theta_e;
command.reference_dq_execution = zhou_iter11_ref.controller.sequence_equivalent_dq_voltage(command, ...
    cfg.vectors,input.theta_e,input.omega_e,1,cfg.paper.Ts_s,cfg.assumptions.candidate_voltage_frame_mode);

predicted_k2 = zhou_iter11_ref.controller.predict_k2(predicted_k1, Fhat, ...
    command.reference_dq_execution, alpha, cfg.paper.Ts_s);
[Jd_eq5, Jq_eq5] = zhou_iter11_ref.controller.independent_costs( ...
    input.reference_dq, predicted_k2);
[Jd_eq7, Jq_eq7] = zhou_iter11_ref.controller.independent_costs(input.reference_dq,predicted_k2);

crosscheck_error = max(abs([Jd_eq5-Jd_eq7, Jq_eq5-Jq_eq7]));
assert(crosscheck_error <= 1e-9, 'ZhouIter11Ref:CostCrosscheckFailure', ...
